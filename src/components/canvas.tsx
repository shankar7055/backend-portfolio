...
const TurbulenceCanvas = forwardRef<TurbulenceCanvasRef, TurbulenceCanvasProps>(
  ...
  const getCoordinates = useCallback((event: MouseEvent | TouchEvent): Point => {
    ...
    // Account for -2 degree rotation with offset correction
    const centerX = rect.width / 2
    const centerY = rect.height / 2
    const angle = 2 * Math.PI / 180 // 2 degrees in radians (positive to counter the -2deg rotation)
    ...
    const finalX = rotatedX + centerX - 6 // Adjust for right shift
    const finalY = rotatedY + centerY - 6 // Adjust for bottom shift
  ...
  const startTurbulenceAnimation = useCallback(() => {
    ...
    animationIdRef.current = requestAnimationFrame(animate)
  }, [allStrokes.length, isDrawing, redrawCurrentStroke, redrawAllStrokes])
